"""Emby Media Server integration for Unfolded Circle Remote. :copyright: (c) 2026 by Meir Miyara. :license: MPL-2.0"""
import asyncio
import json
import logging
import os
...
async def main() -> None:
    ...
    _LOG.info("Starting Emby integration v%s", __version__)
    ...
    config_manager = BaseConfigManager(
        ...
        config_class=EmbyDeviceConfig,
    )
    driver.config_manager = config_manager

    setup_handler = EmbySetupFlow.create_handler(driver)
    ...
    await driver.api.set_device_state(DeviceStates.CONNECTED if device_count > 0 else DeviceStates.DISCONNECTED)
    _LOG.info("Emby integration started - %d server(s) configured", device_count)
